    ...
    <div className="bg-card-light rounded-2xl border border-pink-soft/40 px-6 py-8 flex flex-col gap-5 max-h-[80vh] overflow-y-auto">
      <div className="flex items-baseline gap-3 flex-wrap">
        ...
        {order.order.orderType && (
          <span
            className={`px-3 py-1 rounded-full text-xs font-medium ${order.order.orderType === "mesa"
      ...
      <div className="flex flex-col gap-3">
        ...
        {isLoadingDetails ? (
          ...
          items.map((item) => (
            <div
              key={item.id}
              className="flex flex-col py-2 border-b border-pink-soft/10 last:border-0"
            >
          ...
          <p className="text-sm text-text-main/70 font-medium">
            {formatPrice(Number(item.pricingSnapshot))}
          </p>
        </div>
            ...
            {item.modifiers.map((mod) => {
              if (mod.buildYourOwn) {
                return (
                  ...
                  >
                    {mod.productVariantsTable?.name ?? "Base"}
                  </span>
                );
              }
              if (mod.quantityDelta > 0) {
                return (
                  <span
                    key={mod.id}
                    className="text-sm font-medium text-text-main/80"
                  >
                    + {mod.productVariantsTable?.name ?? "Extra"}
                  </span>
                );
              }
              return null;
